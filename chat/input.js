    ...
    FList.Chat.Input.Commands.roll = {
        func: function(args) {
            ...
            wsSend('RLL ' +
                   JSON.stringify({channel: channel.id, dice: dice}));
    ...
    FList.Chat.Input.Commands.bottle = {
        func: function() {
            ...
            wsSend('RLL ' +
                   JSON.stringify({channel: curTab.id, dice: 'bottle'}));
    ...
    FList.Chat.Input.Commands.ignore = {
        func: function(args) {
            ...
            if (args[0]) {
                wsSend('IGN ' +
                       JSON.stringify({action: 'add', character: args[0]}));
            } else {
                wsSend('IGN ' +
                       JSON.stringify({action: 'add', character: curTab.id}));
            }
    ...
    /**
     * Set auto-response for the currently active status
     *
     * @params {Array} [args] Array of requested arguments
     */
    FList.Chat.Input.Commands.autoresponse = {
        func: function(args) {
            if (args[0].length > 255) {
                return fail('Your auto-response\'s length cannot exceed 255 characters in length.');
            }
            ...
            if (args[0].length === 0) {
                ...
                return fprint('Auto-response cleared.');
            }


            if (FList.Chat.Status.lastStatus.status === "online" ||
                    FList.Chat.Status.lastStatus.status === "looking") {
                return fail('You cannot set your auto-response active while you are currently set as \'Online\' or \'Looking\'.');
            }

            FList.Chat.Status.response.msg = args[0];

            FList.Chat.Status.response.alertedUsers = {};

            fprint('Your auto-response to your current status (\'' +
                   FList.Chat.Status.lastStatus.status + '\') was set to: \'' +
                   FList.Chat.Input.sanitize(args[0]) + '\'');

            pass();
        },
        title: 'AutoResponse',
        does: 'Changes the default response to users that message you for your currently active status.',
        params: [
            {
                type: 'string',
                ID: 'Message',
                hint: 'A message to display as an auto-response to a user if they message you ' +
                'with your current status. (Character limit: 255)'
            }
        ]
    };
